                     ...
                     int a_ncomp)
{
    ...
    m_alpha_coeffs.resize(m_num_amr_levels);
    m_beta_coeffs.resize(m_num_amr_levels);
    m_cc_mask.resize(m_num_amr_levels);
    m_eb_phi.resize(m_num_amr_levels);
    m_eb_beta_coeffs.resize(m_num_amr_levels);
    for (int amrlev = 0; amrlev < m_num_amr_levels; ++amrlev)
    {
        m_alpha_coeffs[amrlev].resize(m_num_mg_levels[amrlev]);
        m_beta_coeffs[amrlev].resize(m_num_mg_levels[amrlev]);
        m_cc_mask[amrlev].resize(m_num_mg_levels[amrlev]);
        m_eb_beta_coeffs[amrlev].resize(m_num_mg_levels[amrlev]);
        for (int mglev = 0; mglev < m_num_mg_levels[amrlev]; ++mglev)
        {
            m_alpha_coeffs[amrlev][mglev].define(m_grids[amrlev][mglev],
                                             m_dmap[amrlev][mglev],
            ...
            for (int idim = 0; idim < AMREX_SPACEDIM; ++idim)
            {
                ...
                const int ng = 1;
                m_beta_coeffs[amrlev][mglev][idim].define(ba,
                                                       m_dmap[amrlev][mglev],
                                                       m_ncomp, ng, MFInfo(), *m_factory[amrlev][mglev]);
                m_beta_coeffs[amrlev][mglev][idim].setVal(0.0);
            }
...
MLEBABecLap::setScalars (Real a, Real b)
{
    ...
    if (a == 0.0)
    {
        for (int amrlev = 0; amrlev < m_num_amr_levels; ++amrlev)
        {
            m_alpha_coeffs[amrlev][0].setVal(0.0);
        }
    }
}
...
{
    MultiFab::Copy(m_alpha_coeffs[amrlev][0], alpha, 0, 0, 1, 0);
    m_needs_update = true;
}
...
{
    m_alpha_coeffs[amrlev][0].setVal(alpha);
    m_needs_update = true;
}
    ...
    if (beta[0]->nComp() == ncomp) {
        for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
            for (int icomp = 0; icomp < ncomp; ++icomp) {
                MultiFab::Copy(m_beta_coeffs[amrlev][0][idim], *beta[idim], icomp, icomp, 1, 0);
            }
        }
    } else {
        for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
            for (int icomp = 0; icomp < ncomp; ++icomp) {
                MultiFab::Copy(m_beta_coeffs[amrlev][0][idim], *beta[idim], 0, icomp, 1, 0);
            }
        }
    }
    m_needs_update = true;
...
{
    for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
        m_beta_coeffs[amrlev][0][idim].setVal(beta);
    }
    m_needs_update = true;
...
MLEBABecLap::setBetaCoeffs (int amrlev, Vector<Real> const& beta)
{
    ...
    for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
        for (int icomp = 0; icomp < ncomp; ++icomp) {
            m_beta_coeffs[amrlev][0][idim].setVal(beta[icomp]);
        }
    }
    m_needs_update = true;
    m_beta_loc     = Location::FaceCenter;
...
MLEBABecLap::setEBDirichlet (int amrlev, const MultiFab& phi, const MultiFab& beta)
{
    ...
    if (m_eb_beta_coeffs[amrlev][0] == nullptr) {
        for (int mglev = 0; mglev < m_num_mg_levels[amrlev]; ++mglev) {
            m_eb_beta_coeffs[amrlev][mglev] = std::make_unique<MultiFab>(m_grids[amrlev][mglev],
                                                                      m_dmap[amrlev][mglev],
                                                                      ncomp, 0, MFInfo(),
                                                                      *m_factory[amrlev][mglev]);
        }
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(phi, mfi_info); mfi.isValid(); ++mfi)
    {
        ...
        Array4<Real> const& betaout = m_eb_beta_coeffs[amrlev][0]->array(mfi);
...
MLEBABecLap::averageDownCoeffs ()
{
    ...
    {
        auto& fine_a_coeffs = m_alpha_coeffs[amrlev];
        auto& fine_b_coeffs = m_beta_coeffs[amrlev];

        averageDownCoeffsSameAmrLevel(amrlev, fine_a_coeffs, fine_b_coeffs,
                                      amrex::GetVecOfPtrs(m_eb_beta_coeffs[amrlev]));
        averageDownCoeffsToCoarseAmrLevel(amrlev);
    }

    averageDownCoeffsSameAmrLevel(0, m_alpha_coeffs[0], m_beta_coeffs[0],
                                  amrex::GetVecOfPtrs(m_eb_beta_coeffs[0]));

    for (int amrlev = 0; amrlev < m_num_amr_levels; ++amrlev) {
        for (int mglev = 0; mglev < m_num_mg_levels[amrlev]; ++mglev) {
            for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
                m_beta_coeffs[amrlev][mglev][idim].FillBoundary(m_geom[amrlev][mglev].periodicity());
            }
        }
    }
}
...
{
    auto const& fine_a_coeffs = m_alpha_coeffs[flev  ].back();
    auto const& fine_b_coeffs = m_beta_coeffs[flev  ].back();
    auto      & crse_a_coeffs = m_alpha_coeffs[flev-1].front();
    auto      & crse_b_coeffs = m_beta_coeffs[flev-1].front();
    auto const& fine_eb_b_coeffs = m_eb_beta_coeffs[flev  ].back();
    auto      & crse_eb_b_coeffs = m_eb_beta_coeffs[flev-1].front();
    ...
    {  // No Dirichlet
        for (int alev = 0; alev < m_num_amr_levels; ++alev)
        {
            if (m_domain_covered[alev])
            {
                ...
                {
                    Real asum = m_alpha_coeffs[alev].back().sum();
                    Real amax = m_alpha_coeffs[alev].back().norm0();
    ...
    for (MFIter mfi(mf, mfi_info); mfi.isValid(); ++mfi)
    {
        ...
        else if (fabtyp == FabType::singlevalued)
        {
            Array4<Real const> const& bebfab
                = (is_eb_dirichlet) ? m_eb_beta_coeffs[amrlev][mglev]->const_array(mfi) : foo;
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
            ...
            for (MFIter mfi(*a_flux[amrlev], mfi_info); mfi.isValid(); ++mfi)
            {
                ...
                if (fabtyp == FabType::covered || fabtyp == FabType::regular) {
                    ...
                    Array4<Real const> const& bebfab = m_eb_beta_coeffs[amrlev][mglev]->const_array(mfi);
...
MLEBABecLap::makeHypre (Hypre::Interface hypre_interface) const
{
    ...
    ijmatrix_solver->setEBDirichlet(m_eb_beta_coeffs[0].back().get());
    return hypre_solver;
}
#endif
...
MLEBABecLap::makePETSc () const
{
    ...
    petsc_solver->setEBDirichlet(m_eb_beta_coeffs[0].back().get());
    return petsc_solver;
}
#endif
